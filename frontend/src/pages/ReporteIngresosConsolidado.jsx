...
const ReporteIngresosConsolidado = () => {
    ...
    const fetchMovimientos = async (customFilters = null) => {
        try {
            ...
            const res = await api.get(`/reportes/detalle-ingresos-consolidado?inicio=${f.inicio}&fin=${f.fin}`);
    ...
    const filteredMovimientos = useMemo(() => {
        return movimientos.filter(i => {
            ...
            const matchesColumn = (
                i.hotel.toLowerCase().includes(columnFilters.hotel.toLowerCase()) &&
    ...
    const handleExportPDF = () => {
        ...
        doc.setFontSize(18);
        doc.text('Caja General Consolidada (Plaza + Colonial)', 14, 20);
        doc.setFontSize(10);
        doc.text(`Periodo: ${filtros.inicio} al ${filtros.fin}`, 14, 28);
        
        doc.setFontSize(11);
        doc.text(`Consolidado - Ingresos: ${formatCurrency(stats.consolidado.ingresos)} | Egresos: ${formatCurrency(stats.consolidado.egresos)} | Balance: ${formatCurrency(stats.consolidado.balance)}`, 14, 40);
        ...
        doc.save(`Reporte_Consolidado_${filtros.inicio}_a_${filtros.fin}.pdf`);
    };
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div>
                    ...
                    <div className="flex items-center gap-4 mt-1">
                        <p className="text-gray-500 text-sm font-medium uppercase tracking-wider italic">Auditoría Multi-Propiedad</p>
