...
const CajaDiariaConsolidada = () => {
    ...
    const mergedData = useMemo(() => {
        ...
        // Convertir map a array y ordenar por fecha (asumiendo formato DD/MM)
        // Nota: La ordenación por DD/MM puede ser compleja si cruza años, 
        // pero getStatsFromDB ya los devuelve ordenados cronológicamente.
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="bg-slate-100 p-1.5 rounded-[1.2rem] flex gap-1">
                        <button 
                            ...
                            className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all ${viewMode === 'all' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}
                        >
                            TODOS
                        </button>
                        <button 
                            onClick={() => setViewMode('plaza')}
                            className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all ${viewMode === 'plaza' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}
                        >
                            PLAZA
                        </button>
                        <button 
                            onClick={() => setViewMode('colonial')}
                            className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all ${viewMode === 'colonial' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500 hover:bg-slate-200'}`}
                        >
                            COLONIAL
                        </button>
                    </div>
                            ...
                            {loading ? (
                                <tr>
                                    <td colSpan="13" className="px-6 py-20 text-center">
                                        <div className="flex flex-col items-center justify-center space-y-4">
                                            ...
                                            <p className="text-sm font-black text-slate-400 uppercase tracking-widest">Consolidando registros financieros...</p>
                                        </div>
                                    </td>
                                </tr>
                            ) : mergedData.length === 0 ? (
                                <tr>
                                    <td colSpan="13" className="px-6 py-20 text-center text-slate-400 font-bold italic">
                                ...
                                <>
                                        ...
                                        <tr key={i} className="hover:bg-slate-50/50 transition-colors group">
                                            ...
                                            {(viewMode === 'all' || viewMode === 'plaza') && (
                                                <>
                                                    ...
                                                    <td className={`px-6 py-4 font-black text-sm ${d.plaza.neto >= 0 ? 'text-blue-600' : 'text-rose-600'}`}>
                                                        ${formatCurrency(d.plaza.neto)}
                                                    </td>
                                                    <td className="px-6 py-4 text-center">
                                                        <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black ${d.plaza.pct >= 0 ? 'bg-blue-50 text-blue-600' : 'bg-rose-50 text-rose-600'}`}>
                                                            {d.plaza.pct.toFixed(1)}%
                                                        </span>
                                                    </td>
                                                </>
                                            )}
                                            ...
                                            {(viewMode === 'all' || viewMode === 'colonial') && (
                                                <>
                                                    ...
                                                    <td className={`px-6 py-4 font-black text-sm ${d.colonial.neto >= 0 ? 'text-orange-600' : 'text-rose-600'}`}>
                                                        ${formatCurrency(d.colonial.neto)}
                                                    </td>
                                                    <td className="px-6 py-4 text-center">
                                                        <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black ${d.colonial.pct >= 0 ? 'bg-orange-50 text-orange-600' : 'bg-rose-50 text-rose-600'}`}>
                                                            {d.colonial.pct.toFixed(1)}%
                                                        </span>
                                                    </td>
                                                </>
                                            )}
                                            ...
                                            {viewMode === 'all' && (
                                                <>
                                                    ...
                                                    <td className={`px-6 py-4 font-black text-sm bg-slate-50/30 ${d.total.neto >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                        ${formatCurrency(d.total.neto)}
                                                    </td>
                                                    <td className="px-6 py-4 text-center bg-slate-50/30">
                                                        <div className="flex items-center justify-center gap-1">
                                                            {d.total.pct >= 0 ? <TrendingUp size={12} className="text-emerald-500" /> : <TrendingDown size={12} className="text-rose-500" />}
                                                            <span className={`text-xs font-black ${d.total.pct >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                                                                {d.total.pct.toFixed(1)}%
                                                            </span>
                                                        </div>
                                                    </td>
                                                </>
                                    ...
                                    <tr className="bg-slate-900 text-white">
                                        <td className="px-6 py-5 font-black text-xs uppercase tracking-widest">TOTALES</td>
                                        
                                        {(viewMode === 'all' || viewMode === 'plaza') && (
                                            <>
                                                <td className="px-6 py-5 font-black text-sm">${formatCurrency(totals.plaza.ingresos)}</td>
                                                <td className="px-6 py-5 font-bold text-slate-400 text-sm">${formatCurrency(totals.plaza.egresos)}</td>
                                                <td className="px-6 py-5 font-black text-sm text-blue-400">${formatCurrency(totals.plaza.neto)}</td>
                                                <td className="px-6 py-5 text-center font-black text-xs">{totalPcts.plaza.toFixed(1)}%</td>
                                            </>
                                        )}

                                        {(viewMode === 'all' || viewMode === 'colonial') && (
                                            <>
                                                <td className="px-6 py-5 font-black text-sm">${formatCurrency(totals.colonial.ingresos)}</td>
                                                <td className="px-6 py-5 font-bold text-slate-400 text-sm">${formatCurrency(totals.colonial.egresos)}</td>
                                                <td className="px-6 py-5 font-black text-sm text-orange-400">${formatCurrency(totals.colonial.neto)}</td>
                                                <td className="px-6 py-5 text-center font-black text-xs">{totalPcts.colonial.toFixed(1)}%</td>
                                            </>
                                        )}

                                        {viewMode === 'all' && (
                                            <>
                                                <td className="px-6 py-5 font-black text-sm text-indigo-300">${formatCurrency(totals.total.ingresos)}</td>
                                                <td className="px-6 py-5 font-bold text-slate-400 text-sm">${formatCurrency(totals.total.egresos)}</td>
                                                <td className="px-6 py-5 font-black text-sm text-emerald-400">${formatCurrency(totals.total.neto)}</td>
                                                <td className="px-6 py-5 text-center font-black text-sm bg-white/10">{totalPcts.total.toFixed(1)}%</td>
                                            </>
                                        )}
                                    </tr>
                                </>
